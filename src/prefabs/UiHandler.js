class UiHandler extends Phaser.GameObjects.Sprite {
    constructor(scene) {
        ...
        //score text
        this.score = scene.add.bitmapText(96, game.config.height*1/12, 'pixelU', game.playerStats.score, 32).setScrollFactor(0).setOrigin(0.5)
        //bones
    ...
    updateHelmets(value) {
        this.helmet1.setAlpha(0)
        this.helmet2.setAlpha(0)
        this.helmet3.setAlpha(0)
        for (let i = 1; i <= value; i++) {
            this[`helmet${i}`].setAlpha(1)
        }
    }

    updateBones(value) {
        this.bone1.setAlpha(0)
        this.bone2.setAlpha(0)
        this.bone3.setAlpha(0)
        this.bone4.setAlpha(0)
        this.bone5.setAlpha(0)
        for (let i = 1; i <= value; i++) {
            this[`bone${i}`].setAlpha(1)
        }
    }
}